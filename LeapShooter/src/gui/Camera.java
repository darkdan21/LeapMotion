...
public class Camera {
	...
	/**
	 * Prepare an orthographic projection matrix based on the bounding region defined by (right, left, top, bottom) given 
	...
	/**
	 * This method prepares the view matrix for use in rendering.
	 * The view matrix represents the position of the virtual camera in 3D space.
	 * 	  - Reset view matrix to identity matrix
	 *    - Convert from spherical coordinates to Cartesian view-direction unit vector
	 *    - Pass Vectors into lookAt method to build matrix.
	 *    - Buffer the matrix so it can be streamed to the GPU.
	 */
	...
	public void setProjection3D( Shader s ){
		...
		matrixView.store( bufferedMatrixView );
        bufferedMatrixView.flip();
        glUniformMatrix4( s.getUniformLocationMatrixViewInverse(), false, bufferedMatrixView );*/
	}
	
	/**
	 * Configures the specified shader 'shader' for 2D rendering by passing the buffered, orthographic projection matrix
	 * to the shader.
	 * @param s
	 */
    ...
    // Update the camera. This allows the camera to be moved, rotated (or whatever else is necessary)
    public void update() { 
        ...
        if ( Mouse.isButtonDown( 2 ) && Display.isActive()){
        	...
        	int cx = clickX;//this.scapp.getWindowWidth()/2;
        	int cy = clickY;//this.scapp.getWindowHeight()/2;
        ...
        cameraLookAt.set( (float)Math.cos(  yaw+Math.PI/2.0 ), (float)Math.sin( yaw+Math.PI/2.0 ), (float)Math.sin( pitch-Math.PI/2.0 ));
        ...
        if ( Keyboard.isKeyDown( Keyboard.KEY_A )){
            ...
            /*Strafe left*/ Vector3f.sub( position, direction, position ); 
        }
        if ( Keyboard.isKeyDown( Keyboard.KEY_D )){
            direction.set( (float)Math.cos(  yaw )*speed, (float)Math.sin( yaw )*speed, 0);
            /*Strafe right*/Vector3f.add( position, direction, position ); 
        }
        ...
        if ( Keyboard.isKeyDown( Keyboard.KEY_P )) {
            ...
            direction.set( (float)Math.cos(  yaw+Math.PI/2.0 ), (float)Math.sin( yaw+Math.PI/2.0 ), (float)Math.sin( pitch+Math.PI/2.0 ));
 
        
        // Smooth
        positionSmooth.x += (position.x - positionSmooth.x)*0.10;
        positionSmooth.y += (position.y - positionSmooth.y)*0.10;
        positionSmooth.z += (position.z - positionSmooth.z)*0.10;
        
        // Clamp coordinates
        if ( position.x > -50 ) { position.x = -50; }
        if ( position.x < -950 ) { position.x = -950; }
        if ( position.y < 100 ) { position.y = 100; }
        if ( position.y > 820 ) { position.y = 820; }
    ...
    /**
     * Converts a point in 3D space to a point in 2D space. The X,Y components of the
     * @param position3D
     * @return returns a vec3( x-coordinate on screen, y-coordinate on screen, depth-bias from camera)
     */
    public Vector3f projectTo2D( Vector3f position3D ){
        ...
        if ( depth >= 0.0f && depth <= 1.0f ){
            return new Vector3f( ((float)ss.x * 0.5f + 0.5f) * (float)Display.getWidth(), (1.0f-((float)ss.y * 0.5f + 0.5f)) * (float)Display.getHeight(), depth);
        } else {
            // In the event that the depth is incorrect, i.e the position has been back-projected, this will prevent the coordinate from being re-scaled into the normal range.
    ...
    public int[] getScreenCoords(double x, double y, double z) {
        ...
        boolean result = GLU.gluProject((float) x, (float) y, (float) z, this.bufferedMatrixView, this.bufferedMatrixProjection, viewport, screenCoords);
        if (result) {
